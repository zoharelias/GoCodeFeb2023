...
export const Product = ({ src, title, price, category }) => {
  ...
  const addProduct = () =>{
    ...
    if(isProductAlreadyInCart === undefined){ //this product is not in cart yet
      ...
      //shoppingCart.splice(isProductAlreadyInCart,1);
      isProductAlreadyInCart.amount += 1;
      setShoppingCart([...filteredArr,isProductAlreadyInCart]);
    }
    ...
    return (
      <div className="product-card">
        ...
        <div className="product-info">
          ...
          <h5>{category}</h5>
          <h6>{price}</h6>
          {/* <button onClick={() => {setShoppingCart(prev => parseInt(prev.amount) + 1 )}}>Add To Shopping Cart</button> */}
          <button onClick={addProduct}>Add To Shopping Cart</button>
          <button onClick={removeProduct}>Remove from Shopping Cart</button>
        </div>
      </div>
    );
  };
